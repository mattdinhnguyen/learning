...
from rb_tree import Node, Root, BLACK, RED
...
class RbTreeTests(unittest.TestCase):
    ...
    def test_find_node(self):
        ...
        """
        # valid cases
        self.assertEqual(Root.find_node(5), node_5)
        self.assertEqual(Root.find_node(2), node_2)
        self.assertEqual(Root.find_node(1), node_1)
        self.assertEqual(Root.find_node(4), node_4)
        self.assertEqual(Root.find_node(3), node_3)
        self.assertEqual(Root.find_node(7), node_7)
        self.assertEqual(Root.find_node(6), node_6)
        self.assertEqual(Root.find_node(9), node_9)
        self.assertEqual(Root.find_node(15), node_15)
        # invalid cases
        self.assertIsNone(Root.find_node(-1))
        self.assertIsNone(Root.find_node(52454225))
        self.assertIsNone(Root.find_node(0))
        self.assertIsNone(Root.find_node(401))
        self.assertIsNone(Root.find_node(3.00001))
    ...
    def test_recoloring_only(self):
        ...
        node_8 = Node(value=8, color=RED, parent=node_6) #OK
        node_2 = Node(value=2, color=RED, parent=node_6) #OK
        node_6.left = node_2 #OK
        node_6.right = node_8 #OK
        node_m20 = Node(value=-20, color=BLACK, parent=node_m10) #OK
        node_m10.left = node_m20 #OK
        node_m10.right = node_6 #OK

        # RIGHT SUBTREE
        node_20 = Node(value=20, color=RED, parent=Root) #OK
        node_15 = Node(value=15, color=BLACK, parent=node_20) #OK
        node_25 = Node(25, color=BLACK, parent=node_20) #OK
...
'''
 ...
 Add-> -22R                                                        22R
    ...
    def test_delete_single_deep_child(self):
        ...
        self.assertIsNone(Root.find_node(49))  # assure its not in the tree
  ...
  REMOVE--->    ___10B___           parent is black             ___20B___
    ...
    def test_add_delete_random_order(self):
        ...
        """
        rb_tree = RedBlackTree()
        Root.AddNode(Node(90)
        Root.AddNode(Node(70)
        Root.AddNode(Node(43)
        rb_tree.remove(70)
        Root.AddNode(Node(24)
        Root.AddNode(Node(14)
        Root.AddNode(Node(93)
        Root.AddNode(Node(47)
        rb_tree.remove(47)
        rb_tree.remove(90)
        Root.AddNode(Node(57)
        Root.AddNode(Node(1)
        Root.AddNode(Node(60)
        Root.AddNode(Node(47)
        rb_tree.remove(47)
        rb_tree.remove(1)
        rb_tree.remove(43)
        Root.AddNode(Node(49)
        """
